...
public class BasicGenerator implements IPolyominoGenerator {
	...
	public IPolyomino getNext() {
		...
		if (pieceSelection < 3) {
			...
			return new hw3.IPiece(new hw3.api.Position(-2, 5), getRandColors(3));
		} else if (pieceSelection == 3) {
			// This code path has a 20% probability
			return new hw3.DiagonalPiece(new hw3.api.Position(-1, 5), getRandColors(2));
		} else {
			// This code path has a 20% probability
			return new hw3.LPiece(new hw3.api.Position(-2, 5), getRandColors(4));
		}
	}
